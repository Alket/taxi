...
import { applyBlogPostJsonToPage } from "@/lib/blog/apply-blog-post-json"
import { pageContentToBlogPost } from "@/lib/blog/cms"
import { parseBlogPostJsonText } from "@/lib/blog/blog-post-schema"
...
export function BlogJsonDialog({
  ...
  async function copyExport() {
    try {
      ...
      toast.success("Blog JSON copied")
    } catch {
  ...
  function applyImport() {
    let post
    try {
      post = parseBlogPostJsonText(draft)
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Invalid blog JSON")
      return
    }
    ...
    const { page: next, slugMismatch, jsonSlug, pageSlug } =
      applyBlogPostJsonToPage(page, post)
      ...
      <DialogContent className="flex max-h-[90vh] flex-col gap-0 overflow-hidden sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Blog JSON · {localeLabel}
          </DialogTitle>
          <DialogDescription>
            Export this language’s post as BlogPost JSON, or paste a translation
            and apply it to the editor. Changes are local until you Save.
          </DialogDescription>
        </DialogHeader>
